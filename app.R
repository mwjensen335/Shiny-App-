...
# set to data folder
#setwd("C:/Users/gabe2/Desktop/Data analysis projects/mavs shiny app/mavs shiny app/data")

# read in whatever CSV is in the folder programatically
...
# loop through csv's to catch any NA dates and replace them with the most common date in that df
for (i in filelist){
      
      df <- fread(i)
      table <- table(df$Date)
      freq_date <- names(table)[which.max(table)]
      df$Date[is.na(df$Date)] <- as.Date(freq_date, format = "%Y-%m-%d")
      df$Date <- as.Date(df$Date, format = "%Y-%m-%d")
      df <- subset(df, select = -UTCDate)
      all_games <- append(all_games, list(df))
      #print(i)
}

#create a merged df
df_merged <- bind_rows(all_games) %>%
      filter(PitcherTeam == "UMA_AMH")
      select(
            Date, GameID, PitchNo, PitchofPA, Inning,`Top/Bottom`,  
            Pitcher, PitcherThrows, PitcherTeam, Batter, BatterSide, BatterId, BatterTeam,
            TaggedPitchType, AutoPitchType, Outs, Balls, Strikes, PitchCall, KorBB, 
            OutsOnPlay, RunsScored,
            VertRelAngle, HorzRelAngle, VertBreak,
            PlayResult, ExitSpeed, Angle, TaggedHitType,
            Distance, Bearing, HitSpinRate,
            RelSpeed, ZoneSpeed, PlateLocHeight, PlateLocSide, InducedVertBreak, HorzBreak,  SpinRate, 
            Tilt, SpinAxis, RelHeight, RelSide, Extension, VertApprAngle, HorzApprAngle,
            HomeTeam, AwayTeam, Stadium, 
            Catcher, CatcherId, CatcherTeam,
            PopTime, ExchangeTime, ThrowSpeed, BasePositionX, BasePositionY, BasePositionZ,
            ThrowPositionX, ThrowPositionY, ThrowPositionZ,
            CatchPositionX, CatchPositionY, CatchPositionZ,
            ay0, ax0,  az0, PAofInning, PitcherId,
      )
